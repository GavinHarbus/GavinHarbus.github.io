...
![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj43wwc2rj30ig0bmwhw.jpg)  

各功能使用的运输层服务：  
![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj45oodb7j30eo0c5div.jpg)
...
+ 停止-等待协议  
	...
	![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj5ad0h3uj30i408575n.jpg)
	确认丢失和确认迟到的情况如下：  
	![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj5bfa5mqj30kg0850uj.jpg)
+ 连续ARQ协议  
	发送方维持一个**发送窗口**，窗口内的分组都能连续发送，不需要等待确认，收到一个分组的确认，则发送窗口向前移动一格
	![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj5inu7x3j30ex067gmp.jpg)
	接收方采用**累积确认**，即不需要对每一个分组进行确认，可以收到几个连续分组后，对最后一个分组进行确认，表示这个分组及其之前的分组都到达了
+ 可靠传输的实现——滑动窗口机制  
	![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj5y7ekk5j30j805n403.jpg)  

滑动窗口也用于流量控制，流量控制就是让发送方控制发送速率，免得接收方来不及接收，这里规定发送窗口大小不得超过接收窗口大小，这里接收方会通过（rwnd）不断提醒发送目前接收方接收窗口大小
![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj6t0aub6j30id09cwh6.jpg)

#### TCP拥塞控制

发送方维持一个拥塞窗口

![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj73ac1ddj30kb07fgna.jpg)
...
+ 快重传  
	![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj768hyavj30i407qta8.jpg)
	如果只是一时丢失确认造成超时，也适用慢开始算法的话，就太浪费资源了，因此接收方发送确认之后如果规定时间内未收到新的分组，则立刻重发确认三次。发送方如果收到连续三次确认，说明并未拥塞，只是偶然，因此执行快恢复算法，提高阈值，之后再进行避免拥塞
+ 快恢复

#### 三次握手

![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj7hmevoaj30gm09zjtq.jpg)  
...
![](https://crescentrose.blob.core.windows.net/blog-img/00831rSTgy1gcj7szpfj3j30i80c4q5z.jpg)
