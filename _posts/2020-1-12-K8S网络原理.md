...
+ 网络命名空间  
	为了支持网络协议栈的多个实例，Linux在网络栈中引入命名空间，实现对不同协议栈的隔离，两个不同命名空间彼此无法通信，同时也可以拥有独立的路由表、防火墙、NAT等。	![](https://crescentrose.blob.core.windows.net/blog-img/006tNbRwgy1gau2ghiiu2j30kd09ita8.jpg)
+ Veth设备对  
...
![](https://crescentrose.blob.core.windows.net/blog-img/006tNbRwgy1gau2yqq3bqj306t0bbq3y.jpg)
...
+ 容器与容器之间直接通信  
	![](https://crescentrose.blob.core.windows.net/blog-img/006tNbRwgy1gau3in5uzkj308r0bhac2.jpg)  
	共享同一网络栈，不需要多做修改，直接使用localhost进行通信
+ pod到pod之间通信
	+ 同一node
		![](https://crescentrose.blob.core.windows.net/blog-img/006tNbRwgy1gau3kh7oo1j30gs0dcaep.jpg)  
		通过docker0网桥进行通信
	+ 不同node
		![](https://crescentrose.blob.core.windows.net/blog-img/006tNbRwgy1gau3mbdqe7j30h20ddtd3.jpg)  
